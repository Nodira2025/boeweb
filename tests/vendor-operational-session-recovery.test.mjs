...
test('Portal vendedor: los flujos críticos recuperan la sesión antes de operar', () => {
  ...
  assert.match(vendorSource, /async function runAssistantLocationSave[\s\S]*?await ensureVendorOperationalSession\(\{ showLogin: true \}\)/);
...
test('Portal vendedor: el cachebuster fuerza a descartar el código de sesión anterior', () => {
  assert.match(vendorHtml, /saas-auth\.js\?v=session_recovery_v4/);
  assert.match(vendorHtml, /operational-api\.js\?v=session_recovery_v3/);
  assert.match(vendorHtml, /vendedor\.js\?v=central_batch_location_v1/);
});
