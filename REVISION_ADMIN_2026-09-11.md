...
1. **Publicación global confiable.** Mantener a Supabase como autoridad incluso si un navegador guarda una revisión local numéricamente mayor. Hoy `createRepository.load()` puede preferir esa copia local; eso permite discrepancias entre admin y tienda. Incorporar detección de cambios simultáneos, historial de publicaciones, comparación de cambios y restauración de una versión anterior. No basta con mostrar “Publicado”.
...
La recomendación es consolidar **Configuración general** con identidad/contenido, proveedores/catálogo, cajas, usuarios/permisos, pagos y estado del sistema. Primero haría publicación confiable y contenido global; después, el flujo completo de encargos nacionales.
