...
BEGIN
  ...
  END IF;
  IF v_draft.stock_quantity > 0 AND v_location_id IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = '23503', MESSAGE = 'El stock inicial requiere una ubicacion central.';
  END IF;
  IF v_draft.stock_quantity > 0 THEN
    v_receipt := public.receive_inventory_v2(
      p_tenant_id, v_product_id, v_location_id, v_draft.stock_quantity,
